...
graph = Graph('http://localhost:7474/browser/', username='neo4j', password='li19960328')

book = xlrd.open_workbook('外文.xlsx')
...
def subclassRelationship(worksheetname, className1,className2):
    ...
    for i in range(len(list1)):
        num+=1
        graph.run("match(a:%s),(b:%s)  where a.name='%s' and b.name='%s'  CREATE (a)-[r:%s]->(b)"%(className1,className2,str(list1[i]),str(list2[i]),str(relationship)))
    #print('创建%d个关系成功'%(num))

CreateNodesBySheetNums(4)
subclassRelationship(workSheetName[4],"topic","dalei")
subclassRelationship(workSheetName[5],"dianguci","topic")
subclassRelationship(workSheetName[6],"dianguci","cixing")
#subclassRelationship(workSheetName[10],"Evolution","EmperorYear")
#subclassRelationship(workSheetName[11],"Evolution","Predecessor")
#subclassRelationship(workSheetName[12],"EmperorYear","Erayear")
