...
if __name__ == '__main__':
    # 读入文本数据
    f = open('1-1.txt', encoding='utf-8')  # 输入已经预处理后的文本
    ...
    lda = models.LdaModel(corpus_tfidf, num_topics=num_topics, id2word=dictionary,
                          alpha='auto',eta=0.01, minimum_probability=0.001,
                          update_every=1, chunksize=100, passes=1,iterations=100)
    ...
    for topic_id in range(num_topics):
        print('主题#%d：\t' % topic_id)
        term_distribute_all = lda.get_topic_terms(topicid=topic_id,topn=num_show_term)  # 所有词的词分布
