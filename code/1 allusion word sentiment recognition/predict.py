...
class Config(object):
    ...
    def __init__(self):
        ...
        self.class_list = ['negtive', 'positive']          # 类别名单
...
def final_predict(config, model, data_iter):
    ...
    with torch.no_grad():
        for texts, _ in data_iter:
            outputs = model(texts)
            #print(outputs.data[None, :].ndim)
            #pred = torch.max(outputs.data, 1)
            #print(pred)
            pred = torch.max(outputs.data[None, :], 1)[1].cpu().numpy()
            #print(pred)
...
def main(text):
    fileout = open('预测结果.txt','w',encoding='utf-8')
    config = Config()
    ...
    for i, j in enumerate(result):
        result1 = 'text:{}'.format(text[i])
        #print(result1.strip('\r\n'))
        result2 = 'label:{}'.format(j)
        #print(result2)
        result3 = result1.strip('\r\n') + ' ' + result2
        #print(result3)
        fileout.write(str(result3)+'\r')


if __name__ == '__main__':

    with open('yuce.txt', "r",encoding='utf-8') as f:
        lines = f.readlines()
        main(lines)
